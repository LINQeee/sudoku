async function init() {
  let backendUrl
  try {
    await fetch('http://localhost:80/ping')
  ...
  function renderBoard(b) {
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        cell.onclick = () => selectCell(r, c, b[r][c].fixed || b[r][c].solved)
  ...
  function renderNumberPad() {
    ...
    for (let n = 1; n <= 9; n++) {
      ...
      // при клике: ставим активный визуал, вызываем placeNumber и подсвечиваем числа
      btn.onclick = () => {
        // активируем кнопку визуально на 700ms
        btn.classList.add('active')
        setTimeout(() => btn.classList.remove('active'), 700)

        // UX: подсветка всех таких чисел (на клиенте)
